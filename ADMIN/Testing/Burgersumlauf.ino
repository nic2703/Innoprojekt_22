...
void setup() {
  ...
  int a=3, s = 255;
  double correction = 19/11.0;
  
  for (int i=0;i<5;++i){
      setSpeed(SPEED_A, DIR_A, HIGH, s);
      brakingProcedure(BRAKE_A, LOW);
      delay(correction*a*500);

      brakingProcedure(BRAKE_A, HIGH);
      delay(500);

      setSpeed(SPEED_B, DIR_B, HIGH, s);
